...
function saveReadingToFirebase() {
    ...
    // Recompute and save daily averages for today
    saveDailyAverage(key);
}

// ---------- Daily Averages ----------
function saveDailyAverage(dateKey) {
    if (!isConnected || !currentUser) return;

    // Helper to crunch entries into averages and write to /dailyAverages
    function computeAndWrite(entries) {
        if (!entries.length) return;
        const indoor = [], outdoor = [], light = [];
        entries.forEach(e => {
            if (e.indoor  !== null && e.indoor  !== undefined && !isNaN(e.indoor))  indoor.push(e.indoor);
            if (e.outdoor !== null && e.outdoor !== undefined && !isNaN(e.outdoor)) outdoor.push(e.outdoor);
            if (e.light   !== null && e.light   !== undefined && !isNaN(e.light))   light.push(e.light);
        });
        const avg = arr => arr.length ? parseFloat((arr.reduce((a, b) => a + b, 0) / arr.length).toFixed(2)) : null;
        const summary = {
            date:       dateKey,
            avgIndoor:  avg(indoor),
            avgOutdoor: avg(outdoor),
            avgLight:   avg(light),
            readings:   Math.max(indoor.length, outdoor.length, light.length),
            updatedAt:  Date.now()
        };
        database.ref('/dailyAverages/' + dateKey).set(summary)
            .then(() => {
                console.log('Daily average saved:', dateKey, summary);
                // Refresh the panel list if it is currently visible
                const panel = document.getElementById('statsFloatingPanel');
                if (panel && panel.style.display !== 'none' && panel.style.opacity !== '0') {
                    renderDailyAverages();
                }
            })
            .catch(err => console.warn('Daily average save failed:', err.message));
    }

    // Try nested date key first (normal structure)
    database.ref('/sensorHistory/' + dateKey).once('value').then(snap => {
        if (snap.val() && snap.hasChildren()) {
            const firstChild = snap.val()[Object.keys(snap.val())[0]];
            if (firstChild && (firstChild.ts !== undefined || firstChild.indoor !== undefined)) {
                // Nested structure — collect entries directly
                const entries = [];
                snap.forEach(child => entries.push(child.val()));
                computeAndWrite(entries);
                return;
            }
        }
        // Flat timestamp key fallback — scan entire /sensorHistory
        database.ref('/sensorHistory').orderByKey().once('value').then(fullSnap => {
            if (!fullSnap.val()) return;
            const entries = [];
            fullSnap.forEach(child => {
                const k = child.key;
                if (/^\d{10,15}$/.test(k)) {
                    const e = child.val();
                    const ts = (e && e.ts) ? e.ts : parseInt(k);
                    const d = new Date(ts);
                    const entryDate = d.getFullYear() + '-' +
                        String(d.getMonth()+1).padStart(2,'0') + '-' +
                        String(d.getDate()).padStart(2,'0');
                    if (entryDate === dateKey) entries.push(e);
                }
            });
            computeAndWrite(entries);
        });
    }).catch(err => console.warn('saveDailyAverage read failed:', err.message));
}

function loadDailyAverages(callback) {
    database.ref('/dailyAverages').orderByKey().once('value').then(snap => {
...
function renderDailyAverages() {
    const container = document.getElementById('dailyAveragesContainer');
    if (!container) return;
    container.innerHTML = '<div style="color:#aaa;font-size:0.8rem;text-align:center;padding:8px;">Loading...</div>';

    loadDailyAverages(entries => {
        if (!entries.length) {
            container.innerHTML = '<div style="color:#aaa;font-size:0.8rem;text-align:center;padding:8px;">No daily averages yet — saves every 30 s</div>';
            return;
        }
        container.innerHTML = entries.map(e => {
            ...
            const label = diff === 0 ? 'Today' : diff === 1 ? 'Yesterday' : e.date;
            const indoorStr  = e.avgIndoor  !== null ? e.avgIndoor.toFixed(1)  + ' °C' : '--';
            const outdoorStr = e.avgOutdoor !== null ? e.avgOutdoor.toFixed(1) + ' °C' : '--';
            const lightStr   = e.avgLight   !== null ? e.avgLight.toFixed(1)   + ' %'  : '--';
            return `
            <div style="background:rgba(102,126,234,0.06);border-radius:10px;padding:10px 12px;margin-bottom:8px;">
              <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:6px;">
                <span style="font-weight:700;font-size:0.82rem;color:#333;">📅 ${label}</span>
                <span style="font-size:0.72rem;color:#999;">${e.readings} readings</span>
              </div>
              <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:6px;text-align:center;">
                <div style="background:rgba(102,126,234,0.1);border-radius:7px;padding:6px;">
                  <div style="font-size:0.68rem;color:#888;margin-bottom:2px;">🌡️ Indoor</div>
                  <div style="font-weight:700;font-size:0.82rem;color:#667eea;">${indoorStr}</div>
                </div>
                <div style="background:rgba(246,173,85,0.12);border-radius:7px;padding:6px;">
                  <div style="font-size:0.68rem;color:#888;margin-bottom:2px;">🌡️ Outdoor</div>
                  <div style="font-weight:700;font-size:0.82rem;color:#ed8936;">${outdoorStr}</div>
                </div>
                <div style="background:rgba(236,201,75,0.12);border-radius:7px;padding:6px;">
                  <div style="font-size:0.68rem;color:#888;margin-bottom:2px;">☀️ Light</div>
                  <div style="font-weight:700;font-size:0.82rem;color:#d69e2e;">${lightStr}</div>
                </div>
              </div>
            </div>`;
        }).join('');
        ...
        snap.forEach(daySnap => {
            ...
            if (isOldEnough && wasViewed) {
                removes.push(
                    ...
                    database.ref('/dailyAverages/' + dateKey).remove()
                );
...
function initCharts() {
    ...
    function openPanel() {
        ...
        // Refresh daily averages list
        if (isConnected) renderDailyAverages();
        // Run initial prune
        pruneOldHistory();
    }
    ...
    // Refresh daily averages button
    const refreshAvgBtn = document.getElementById('refreshAvgBtn');
    if (refreshAvgBtn) refreshAvgBtn.addEventListener('click', () => {
        if (isConnected) renderDailyAverages();
    });
